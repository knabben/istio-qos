...
A GitHub Actions workflow MUST verify build and test correctness on every push and pull
request. The workflow MUST enforce the following gates in order; any gate failure MUST
block the PR:

| Gate | Command | Pass condition |
|------|---------|---------------|
| **Build** | `go build ./...` | Binary compiles with zero errors |
| **Vet** | `go vet ./...` | Zero vet findings |
| **Unit + Integration tests** | `make test` | All tests pass; reconciler package coverage ≥ 80% |
| **Race detector** | `go test -race ./...` (non-e2e) | No data races detected |
| **Coverage threshold** | `go tool cover -func cover.out` | `controller` package total ≥ 80% |

Requirements:
- The workflow MUST install the correct Go version from `go.mod` (`go-version-file: go.mod`).
- Integration tests run via `make test`, which bootstraps `setup-envtest` and sets
  `KUBEBUILDER_ASSETS` automatically — no live cluster or `kind` is required.
- The coverage profile (`cover.out`) generated by `make test` MUST be used to enforce the
  80% threshold; the step MUST exit non-zero if the threshold is not met.
- The race-detector run MUST also set `KUBEBUILDER_ASSETS` so envtest-backed tests execute
  (not just unit tests).
- Go module cache and the `bin/` local tool cache MUST be cached between runs to keep
  workflow execution under 5 minutes on a cold runner.
